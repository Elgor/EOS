...
<section class="mb-5 p-4 round-border bg-white shadow">
    <div class="row">
            ...
            <div class="row">
                <div class="col-12 mb-0">
                    <figure class="text-center">
                        <img class="round-border" src="{{ asset('/storage/'.$product->image) }}" style="max-height: 250px; max-width: 300px;">
                    </figure>
                </div>
                <div class="col-12">
                    <div class="row">
                        ...
                        <div class="col-4">
                            <div class="view overlay rounded z-depth-1 gallery-item round-border mb-2">
                                <a href="{{ asset('/storage/'.$image->path) }}" data-lightbox="photos">
                                    <img src="{{ asset('/storage/'.$image->path) }}" class="img-fluid" style="min-height: 115px">
                                </a>
                            </div>
                        </div>
                        @endforeach
        ...
        <div class="col-md-6">
            ...
            <p class="mb-2 text-muted">by <a href="{{ route('seller.detail',$product->seller->id) }}">{{ $product->seller->business_name }}</a>
            </p>
            <div>
                    ...
                    <form class="form" method="POST" action="{{route('order.store')}}">
                        ...
                        <div class="form-group mb-3">
                            <label class="text-md-right pr-1 font-weight-bold" for="negotiation_price">Negotiation
                                Price</label>
                            <div style="width: 100%">
                                <input name="negotiation_price" type="text" class="form-control @error('negotiation_price') is-invalid @enderror" maxlength="15" placeholder="Rp {{ number_format($product->price,0,',','.') }}">
                                @error('negotiation_price')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" style="width: 100%" name="event_plan_id" required>
                                @foreach(Auth::user()->eventPlans as $e)
                                <option value="">Select Event Plan</option>
                                <option value="{{$e->id}}">{{$e->eventName}}</option>
                                @endforeach
                            </select>
                            @else
                            <a class="ml-3 btn btn-outline-info @error('event_plan_id') is-invalid @enderror" href="{{ route('eventplan.index') }}" role="button">Create Event Plan</a>
                            @endIf
                            @error('event_plan_id')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                        ...
                        <div class="mt-3">
                            ...
                            <a class="ml-3 btn btn-outline-info" href="{{ route('compare.add',$product->id) }}" role="button">Compare</a>
                            <a class="ml-3 pt-2" href="{{ route('wishlist.store',$product->id) }}" data-toggle="tooltip" data-placement="top" title="Wishlist">
